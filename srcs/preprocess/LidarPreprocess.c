...
{	
	...
	int X_SIZE = (int)((x_MAX-x_MIN)/x_DIVISION);	// X_SIZE is 700    前左天坐标系，X表示前方，雷达正下方是原点，700表示0-70米的距离范围
	int Y_SIZE = (int)((y_MAX-y_MIN)/y_DIVISION);   // Y_SIZE is 800    左右各40m
	int Z_SIZE = (int)((z_MAX-z_MIN)/z_DIVISION);   // Z_SIZE is 35     下最小2.5m上最高1m
    ...
    {	
		...
		num = fread(data,sizeof(float),num,fp)/4;                     //所有数据/4=点的个数，x-点的x坐标，y-点的y坐标，z-点的z坐标，r表示点属于哪条射线
        //cout<<"number of points: "<<num<<endl;
		//3D grid box index
	
		//height features X_SIZE * Y_SIZE * (Z_SIZE + 1)
		//density feature X_SIZE * Y_SIZE * 1
		std::vector<int> density_map(Y_SIZE*X_SIZE);                //先建立一个一维的vector
	...
	void createTopViewMaps_ros(const void * indatav, float * s_data, int num)
    {	
