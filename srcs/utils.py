...
def get_bev(velo_array, label_list = None, scores = None):
    ...
    if label_list is not None:
        for corners in label_list:
            ...
            plot_corners = plot_corners.astype(int).reshape((-1, 1, 2))    #shape由4*2变更为1*2,其中4*2表示的是四个点的坐标
            # print('plot_corner_size = ', plot_corners.shape)
            # print('plot_corner = ', plot_corners)
            cv2.polylines(intensity, [plot_corners], True, (255, 0, 0), 2)   #画出车的轮廓
            cv2.line(intensity, tuple(plot_corners[2, 0]), tuple(plot_corners[3, 0]), (0, 0, 255), 3)  #画出车头
